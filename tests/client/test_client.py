...
from entree.client.managers import EntreeUserFetcherMixin
from entree.client.middleware import get_user, InvalidUser, InvalidAuth, AuthMiddleware
from entree.client.db.models import EntreeDBUser
...
class TestEntreeDBUser(DBUserTestCase):
    ...
    def test_pickle(self):
        data = {'some': 'data'}
        user = self.UserClass(key='keyfoo', data=data)
        dumped = pickle.dumps(user)

        res = pickle.loads(dumped)
        assert_equals(user.key, res.key)
        assert_equals(user.data, res.data)
        assert_equals(data, res.data)
    ...
    def test_set_key_set_data_dict_value(self):
        user = self.UserClass()
        user.data['prop'] = 'foo'
        assert_equals('foo', user.data['prop'])

    def test_set_data_db(self):
        data = {'foo': 'bar'}
        user = self.UserClass()
        user.data = data
        assert_equals(data, user.data)

    def test_set_data_cache(self):
        data = {'foo': 'bar'}
        user = self.UserClass()
        user.data = data
        assert_equals(data, user.data)
    ...
    def test_data_returns_data(self):
        data = {'foo': 'bar'}
        user = self.UserClass(data=data)
        assert_equals(data, user.data)

    def test_has_data_returns_property(self):
        val = 'bar'
        data = {'foo': val}

        user = self.UserClass(data=data)
        assert_equals(val, user.data['foo'])
...
class TestEntreeCacheUserManager(DBUserTestCase):
    ...
    def test_get_unmatching_user_raises(self):
        self.UserClass.objects.create(key='foo', data=dict(email='foo@bar.cz'))
        assert_raises(ObjectDoesNotExist, lambda: self.UserClass.objects.get(key='bar'))

    def test_get_matching_user(self):
        user = self.UserClass.objects.create(key='foo', data=dict(email='foo@bar.cz'))
...
class TestEntreeUserFetcher(DBUserTestCase):
    ...
    def test_perform_fetch_returns_entreeuser(self, mocked_urlopen):
        ...
        assert_equals('foo@bar.cz', ret.data['email'])
